...
The `plR_read()` function returns a `plR` S3 object containing three data.tables:
...
The `plR_permute()` function performs gene set enrichment testing using a permutation procedure that:
...
The `plR_permute()` function adds new columns to `set.info`:
...
- Function help: `?plR_read`, `?plR_permute`, `?plR_rescale`, `?plR_prune`
