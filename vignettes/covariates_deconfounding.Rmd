...
- `md_method`: Mahalanobis distance method (`"robust"` for rank-based, `"raw"` for Pearson)
- `kernel_boundary`: Genomic distance flanking region to exclude from kernel weights
- `kernel_function`: Kernel function (`"normal"`, `"exponential"`, or `"inverse"`)
- `kernel_scale`: Scaling parameter for kernel decay
- `kernel_weight_max`: Maximum weight for any single gene
...
plr_with_covar <- plr_permute(
  ...
  md_method = "robust",      # Use rank-based robust method
  kernel_boundary = 1e5,        # 100kb flanking region
  kernel_function = "normal",    # Gaussian kernel
  kernel_scale = 2,          # Standard decay
  kernel_weight_max = 0.05,      # Max 5% weight for any gene
  seed = 42,
  verbose = TRUE,
  n.cores = 1
)
...
|-----------|---------|----------------|---------------------|
| `md_method` | `"robust"` | Use `"raw"` for normal-distributed covariates | `"robust"` (always recommended) |
| `kernel_boundary` | 1e5 | Increase for gene-dense regions | 2e5 or higher |
| `kernel_scale` | 2 | Decrease for tighter local regression | 1 |
| `kernel_weight_max` | 0.05 | Decrease to limit influence of single genes | 0.02 or 0.01 |
