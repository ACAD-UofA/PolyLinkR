...
.path_check <- function(oi.path, si.path, so.path, rr.path, vi.path,
   ...
   if (multi.path) { # check for individually labelled files
      ...
      fE <- lapply(fn0, function(ip) file.exists)
      if (!all(fE)) {
         EMSS <- c(EMSS, paste(paste(ip[!fE], sep = ", "), "does not exist\n"))
      }
...
.par_params <- function(n.cores, fut.plan, verbose, ENV) {
   ...
   if (!fut.plan %in% valid.fp) {
      ...
      if (fut.plan %in% c("auto", "Auto", "AUTO")) {
         fut.plan <- ifelse(n.cores == 1, "sequential", "multisession")
      } else {
...
.get_cgm_bins <- function (maxL, minL, cgm.bin, OI) {
   ...
   bin0 <- 1:nBins
   if (any(cN[bin != 1]$N) < cgm.bin) { # at least one bin below required size
